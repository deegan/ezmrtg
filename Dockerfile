...
RUN apk add --update 
RUN apk add bash nginx mrtg git php82-fpm
...
CMD /usr/sbin/php-fpm82 && /usr/sbin/nginx -c /etc/nginx/nginx.conf && /usr/sbin/crond && /mrtg/scripts/wrapper.sh && tail -f /var/log/nginx/mrtg.access.log && wait
